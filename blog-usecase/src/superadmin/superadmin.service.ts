...
import { LoginUserDto } from "./sueradmin.dto";
...
import { AssignRoleDto } from "./assign-roles.dto";
...
export class SuperAdminService {
    ...
    async loginSuperAdmin(loginUserDto: LoginUserDto) {
        ...
        try{
            loginUserDto.password = crypto.createHash('md5').update(loginUserDto.password).digest('hex');
        ...
        if(!user || loginUserDto.password != user.password ){
            ...
            const token= this.jwtService.sign({userId:user.id,roleId:user.role_id});
            return token
        }
    ...
    async assignRoles(assignRoleDto:AssignRoleDto) {
        ...
        const updateResult = await this.userRepository.update(user_id, { role: role_id });
